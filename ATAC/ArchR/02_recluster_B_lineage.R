...
rna_proj <- readH5AD('../B_cell_raw.h5ad',reader = "python")
...
markers_plasma_3_5 <- getMarkerFeatures(
  ArchRProj = atac_proj, 
  useMatrix = "GeneScoreMatrix", 
  groupBy = "Clusters",
  useGroups = "C3",
...
de_plasma_3_5 <- as.data.frame(getMarkers(markers_plasma_3_5, cutOff = "FDR <= 1"))


nclust <- length(unique(atac_proj$Clusters))
fineClust <- c()
fineClust[1] <- 'Naive_B'
fineClust[2] <- 'Memory_B'
fineClust[3] <- 'Plasma'
fineClust[4] <- 'preB'
fineClust[5] <- 'Plasma'
fineClust[6] <- 'Atypical_Memory_B'
fineClust[7] <- 'proB'
fineClust[8] <- 'proB'
fineClust[9] <- 'Naive_B'
...
#atac_proj$group_celltype2 <- paste0(atac_proj$FineClust,'_',atac_proj$group_info2)
