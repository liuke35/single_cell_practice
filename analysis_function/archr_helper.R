...
relabelClusters <- function(proj, clusterName="Clusters"){
  ...
  tabDF$count <- tabDF$count+runif(row_number(tabDF))
...
visualizeClustering <- function(proj, pointSize=0.75, prefix="", clusterName="Clusters", sampleName="Sample", embedding="UMAP", 
  ...
  p3 <- plotEmbedding(proj, colorBy="cellColData", name="group_info", embedding=embedding, plotAs="points", size=pointSize, pal=diseaseCmap, labelMeans=FALSE)
  ...
  plotPDF(p1,p2,p3,p4,p5,p6, name = paste0(prefix,"Plot-UMAP-Sample-Clusters.pdf"), ArchRProj=proj, addDOC=FALSE, width=5, height=5)
  ...
  diseaseBySamp <- fractionXbyY(clustVec, proj$group_info, add_total=TRUE, xname="Cluster", yname="group_info")
